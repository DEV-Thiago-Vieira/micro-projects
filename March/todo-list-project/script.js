...
const goalWeekdayPicker = document.getElementById("goal-weekday-picker");
const goalColorInput = document.getElementById("goal-color-input");
const goalList = document.getElementById("goal-list");
const toggleFinishedGoalsButton = document.getElementById(
  "toggle-finished-goals-btn",
);
...
const toggleFinishedTasksButton = document.getElementById(
  "toggle-finished-tasks-btn",
);
...
const editProgressQuoteButton = document.getElementById(
  "edit-progress-quote-btn",
);
const saveProgressQuoteButton = document.getElementById(
  "save-progress-quote-btn",
);
const clearProgressQuoteButton = document.getElementById(
  "clear-progress-quote-btn",
);
const cancelProgressQuoteButton = document.getElementById(
  "cancel-progress-quote-btn",
);
...
const cancelRemoveGoalButton = document.getElementById(
  "cancel-remove-goal-btn",
);
const confirmRemoveGoalButton = document.getElementById(
  "confirm-remove-goal-btn",
);
...
const getTaskWord = (count) =>
  count === 1 ? t("taskSingular") : t("taskPlural");
const getDayWord = (count) =>
  count === 1 ? t("dayLowerSingular") : t("dayLowerPlural");
...
const applyStaticTranslations = () => {
  ...
  if (closeScheduleButton) {
    closeScheduleButton.setAttribute(
      "aria-label",
      t("closeRecurringSchedules"),
    );
  }
  ...
  const goalTitleLabel = document.querySelector(
    'label[for="goal-title-input"]',
  );
  if (goalTitleLabel) {
  ...
  const goalDurationLabel = document.querySelector(
    'label[for="goal-duration-input"]',
  );
  ...
  const goalIntervalLabel = document.querySelector(
    'label[for="goal-interval-input"]',
  );
  ...
  const goalStartLabel = document.querySelector(
    'label[for="goal-start-date-input"]',
  );
  if (goalStartLabel) {
  ...
  const goalWeekdaysLabel = document.querySelector(
    "#goal-weekdays-field label",
  );
  if (goalWeekdaysLabel) {
    goalWeekdaysLabel.textContent = t("weekdays");
  }
  if (goalWeekdayPicker) {
    goalWeekdayPicker.setAttribute("aria-label", t("weekdays"));
    renderWeekdayPickerButtons(
      goalWeekdayPicker,
      getSelectedWeekdaysFromPicker(goalWeekdayPicker),
    );
  }
  const goalColorLabel = document.querySelector(
    'label[for="goal-color-input"]',
  );
  if (goalColorLabel) {
  ...
  const goalSubmitButton = document.querySelector(
    "#goal-form .schedule-submit-btn",
  );
  if (goalSubmitButton) {
  ...
  const scheduleGoalLabel = document.querySelector(
    'label[for="schedule-goal-select"]',
  );
  ...
  const scheduleIntervalLabel = document.querySelector(
    'label[for="schedule-interval-input"]',
  );
  ...
  const scheduleWeekdaysLabel = document.querySelector(
    "#schedule-weekdays-field label",
  );
  if (scheduleWeekdaysLabel) {
    scheduleWeekdaysLabel.textContent = t("weekdays");
  }
  if (scheduleWeekdayPicker) {
    scheduleWeekdayPicker.setAttribute("aria-label", t("weekdays"));
    renderWeekdayPickerButtons(
      scheduleWeekdayPicker,
      getSelectedWeekdaysFromPicker(scheduleWeekdayPicker),
    );
  }
  const scheduleSubmitButton = document.querySelector(
  ...
  const progressCurrentLabel = document.querySelector(
    "#progress-current-card .progress-stat-label",
  );
  if (progressCurrentLabel) {
    progressCurrentLabel.textContent = t("currentStreakLabel");
  }
  const progressBestLabel = document.querySelector(
    "#progress-best-card .progress-stat-label",
  );
  ...
  if (editProgressQuoteButton) {
    editProgressQuoteButton.setAttribute(
      "aria-label",
      t("editMotivationQuote"),
    );
    editProgressQuoteButton.setAttribute("title", t("editQuote"));
  }
  const progressQuoteLabel = document.querySelector(
    'label[for="progress-quote-input"]',
  );
...
const toggleTheme = () => {
  const nextTheme = document.body.classList.contains("is-dark")
    ? "light"
    : "dark";
...
const getTaskTextElement = (taskItem) =>
  taskItem.querySelector(".todo-item-text");
...
const getDragAutoScrollStep = (pointerY) => {
  ...
  if (pointerY < rect.top + DRAG_SCROLL_EDGE_PX) {
    const intensity =
      (rect.top + DRAG_SCROLL_EDGE_PX - pointerY) / DRAG_SCROLL_EDGE_PX;
...
const isGoalColorInUse = (color, excludedGoalId = null) => {
  ...
  return state.goals.some(
    (goal) =>
      goal.id !== excludedGoalId && getGoalColor(goal) === normalizedColor,
  );
};
...
const normalizeWeekdayList = (value, fallback = ALL_WEEKDAYS) => {
...
const orderWeekdaysForDisplay = (weekdays) => {
...
const formatWeekdaysLabel = (weekdays) => {
...
const getDateStampWeekday = (dateStamp) => {
...
const renderWeekdayPickerButtons = (container, weekdays) => {
  if (!container) {
    return;
  }

  const selectedDays = new Set(normalizeWeekdayList(weekdays));
...
const getSelectedWeekdaysFromPicker = (container) => {
  ...
  const selectedWeekdays = Array.from(
    container.querySelectorAll(".weekday-btn.is-active"),
  )
  ...
  return [...new Set(selectedWeekdays)].sort((a, b) => a - b);
};
  ...
  finishedGoals.forEach((goal) => {
    ...
    meta.textContent = t("endedOn", {
      date: formatDisplayDate(
        goal.finishedAt || getGoalPeriodEndDate(goal) || goal.startDate,
      ),
    });
...
const buildGoalProgressItem = (goal, today, extraClass = "") => {
  ...
  const validDates = trackingDates.filter((dateStamp) =>
    isGoalActiveOnDate(goal, dateStamp),
  );
...
const renderGoalProgress = () => {
  ...
  if (finishedGoals.length > 0) {
    ...
    toggleBtn.textContent = t("showFinishedGoalsProgress", {
